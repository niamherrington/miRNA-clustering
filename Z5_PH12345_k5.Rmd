...
```{r}
survminer::ggsurvplot(
    fit = survival::survfit(Bases1 ~ cluster5, data = ForSurvival), 
    xlab = "Years", 
    ylab = "Overall survival probability",
    legend.title = "",
    palette=c("#88CCEE", "#CC6677", "#DDCC77", "#332288", "#AA4499"),
...
broom::tidy(coxph(Bases1~sex+age+cluster5, data = ForSurvival), exp = TRUE) %>% kable(., caption = "Cox regression: ~ sex + age + 5 clusters") %>% kable_styling(full_width = TRUE)
coxph(Bases5~sex+age+cluster5, data = ForSurvival) %>% gtsummary::tbl_regression(exp = TRUE)

broom::tidy(coxph(Bases1~cluster5, data = ForSurvival), exp = TRUE) %>% kable(., caption = "Cox regression: ~ 5 clusters") %>% kable_styling(full_width = TRUE)
coxph(Bases5~cluster5, data = ForSurvival) %>% gtsummary::tbl_regression(exp = TRUE)
